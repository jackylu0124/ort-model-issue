...
import android.util.Base64; // JL added
import java.io.ByteArrayInputStream;  // JL added
import android.util.Log;  // JL added
...
public class OnnxruntimeModule extends ReactContextBaseJavaModule {
  ...
  // JL commented out the following block of code
  // /**
  ...
  // JL added (essentially the same code as the original one above, only changed variable name in order to be consistent with the function's features)
  @ReactMethod
  public void loadModel(String bufferEncoding, ReadableMap options, Promise promise) {
    try {
      WritableMap resultMap = loadModel(bufferEncoding, options);
      promise.resolve(resultMap);
    } catch (Exception e) {
      promise.reject("Can't read a model " + bufferEncoding, e);
    }
  }
  ...
  // JL added
  public WritableMap loadModel(String bufferEncoding, ReadableMap options) throws Exception {
    InputStream modelStream = new ByteArrayInputStream(Base64.decode(bufferEncoding, Base64.DEFAULT));
    String key = bufferEncoding;  // JL: for the sake of simplicity, use the string storing the base64 buffer encoding as the key
    WritableMap resultMap = loadModel(key, modelStream, options);
    modelStream.close();
    return resultMap;
  }
  ...
  // JL added (essentially the same code as the original one above, only changed variable name in order to be consistent with the function's features)
  public WritableMap loadModel(String key, InputStream modelStream, ReadableMap options) throws Exception {
    OrtSession ortSession = null;
    ...
    if (!sessionMap.containsKey(key)) {
      ...
      try {
        ...
        throw new Exception("Can't read a model " + key, e);
      }
